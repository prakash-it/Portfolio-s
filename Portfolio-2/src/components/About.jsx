...
import resume from '../assets/PRAKASH.pdf'
export default function About() {
  return (
    <section className='max-w-screen-xl px-6 mx-auto py-10 md:py-24' id='about'>
      ...
      <a  href={resume}  download="PRAKSH_Resume.pdf" >
            <InfoCard count={ABOUT.stats.Resume} label="Download Resume" />
          </a>
...
const InfoCard = ({ label, count, children }) => {
  return (
    <div className='bg-gradient-to-br from-blue-950 to-slate-900 rounded border border-blue-800/40 px-4 md:px-6 py-3 transition duration-300 hover:shadow-lg hover:shadow-blue-500/100'>
      {count && <div className="text-primary text-xl md:text-2xl font-semibold">{count}</div>}
      {label && <span className='text-blue-300 text-xs md:text-sm font-normal'>{label}</span>}
